<script>
  ...
  import addStylesToChart from "./helpers/add-styles-chart";

  import { LOGIN_PASSWORD_ERR, JWT_ERROR } from "./errors/messages-constants";
  ...
  const deleteCheckedUser = async (userId) => {
    try {
      ...
      userList.update((users) =>
        $userList.filter((user) => user._id !== data._id)
      );
  ...
  const handleSummary = async () => {
    if (
      moment($currentSummary.date).format("DD.MM.YYYY") ===
  ...
  const routeEventHandler = (data) => {
    ...
    if (
      data.detail.method === "getCertainSummaries" ||
      data.detail.method === "getLastSummaries"
    ) {
...
<body>
    ...
    <div class="header__nav-wrapper">
      ...
      {#if $currentUser.name}
        <p class="header__user-name">
          {$currentUser.name || 'Неопознанный пользователь'}
        </p>

        <span
          class="header__user-admin">{$currentUser.admin ? 'админ' : ''}</span>

        <a
          href="/"
          class="header__nav-link"
          on:click|preventDefault={logOut}>Выйти</a>
      {/if}
    </div>
  </header>
  ...
  <main>
    {#if $currentUser._id}
      <div class="main-actions__wrapper">
        <button class="button button_accent" on:click={() => openModal()}>Внести
          данные</button>
      </div>
    {/if}
    ...
    <Router
      routes={{ '/signin': Signin, '/signup': Signup, '/users': Users, '/data': Summaries }}
  ...
  <Modal bind:this={modal}>
    <div class="modal__data-wrapper">
      <h2>
        Вы вносите
        {$currentSummary.fact.length ? 'фактические' : 'плановые'}
        показатели за
    ...
    <div class="modal__actions">
      <button
        ...
        on:click={handleSummary}>Сохранить и закрыть</button>
  ...
  {#if $isPending}
    <div class="spinner">
      <div class="spinner__wrapper">
        <RingLoader size="150" color="#FF3E00" unit="px" duration="2s" />
      </div>
    </div>
  {/if}
</body>
